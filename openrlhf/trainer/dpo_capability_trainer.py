...
from openrlhf.models import DPOLoss, SFTLoss
from openrlhf.utils.distributed_sampler import DistributedSampler


class DPOCapabilityTrainer(ABC):
    """
    ...
    def __init__(
        ...
        self.capability_loss_fn = SFTLoss()
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens, score_inputs, score_masks, score_loss = data
                ...
                # add SFT (capability) loss 
                score_inputs = score_inputs.to(torch.cuda.current_device()).squeeze(1)
                score_masks = score_masks.to(torch.cuda.current_device()).squeeze(1)
                score_loss = score_loss.to(torch.cuda.current_device()).squeeze(1)
                log_probs, output = self.model(
                    score_inputs,
                    attention_mask=score_masks,
                    return_output=True,
                    return_logprobs=True,
                    ring_attn_group=self.strategy.ring_attn_group,
                )

                # print("log_probs: ", log_probs, "score_loss: ", score_loss)
                if (score_loss[:, :-1] == 0).all():
                    gpt_loss = (log_probs.sum() * 0).to(log_probs.device) 
                else:
                    gpt_loss = self.capability_loss_fn(log_probs, score_loss[:,:-1])
                print("gpt loss: ", gpt_loss)
                # loss function
                ...
                loss = gpt_loss * 0.1 + preference_loss + aux_loss * self.args.aux_loss_coef + nll_loss * self.args.nll_loss_coef
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens, score_ids, score_mask, score_id_lens = data
                chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
